...
namespace SQLiteKei.DataAccess.QueryBuilders
{
    public class SelectQueryBuilder : ConditionalQueryBuilder
    {
        ...
        private Dictionary<string, string> selects = new Dictionary<string, string>();
        private List<OrderData> OrderClauses { get; } = new List<OrderData>();

        public SelectQueryBuilder(string? select = null, string? alias = null)
        {
            if (select != null)
                selects.Add(select, alias ?? string.Empty);
        }
