...
namespace SQLiteKei.Views.UserControls
{
    ...
    public partial class TableRecordsTabContent : UserControl
    {
        ...
        {
            log.Logger.Info("Executing select query from SelectQuery window.\n" + selectQuery);
            try
            {
                var dbPath = SQLite.WPF.Settings.Default.CurrentDatabase;
            ...
            {
                log.Logger.Error("Select query failed.", ex);
